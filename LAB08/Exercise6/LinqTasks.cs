...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(emp => emp.Salary == Emps.Max(emp2 => emp2.Salary));
            return result;
        }
        ...
        {
            var defaultObject = new[] { new { Ename = "Brak wartości", Job = string.Empty, HireDate = (DateTime?)null } };
            IEnumerable<object> result = Emps
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Emps
                .GroupBy(e => e.Deptno)
                .Where(g => g.Count() > 1)
                .Select(g => new { Name = Depts.First(d => d.Deptno == g.Key).Dname, NumOfEmployees = g.Count() });
            return result;
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            ...
            var departmentsWithEmpCounts = Depts
                .GroupJoin(empCounts,
                    dept => dept.Deptno,
                    empCount => empCount.Deptno,
                    (dept, tempCounts) => new { Department = dept, EmpCounts = tempCounts.DefaultIfEmpty() })
                .SelectMany(
                    x => x.EmpCounts,
                    (x, y) => new { x.Department, Count = y?.Count ?? 0 }
                );
            IEnumerable<Dept> result = departmentsWithEmpCounts
